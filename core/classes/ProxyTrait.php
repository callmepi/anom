<?php
/** ProxyTrait
 * implements the proxy design pattern in a trait function
 * 
 * uses: Cache  ( Registry::use('cache') )
 * 
 */
namespace anom\core;
...
trait ProxyTrait
{
     ...
     * ---
     * Def::PROXY_CACHE_ERRORS : cache value even if callback returns error
     * Def::PROXY_IGNORE_CACHE : ignore if cache exist; get result from callback
     * Def::PROXY_DO_NOT_CACHE : do not cache the result (even if from callback)
     */
    function proxy(array $callable, array $args, int $ttl, int $options=0)
    {   
        // resolve option flags
        $cache_errors = ($options & Def::PROXY_CACHE_ERRORS) ? true : false;
        $ignore_cache = ($options & Def::PROXY_IGNORE_CACHE) ? true : false;
        $do_not_cache = ($options & Def::PROXY_DO_NOT_CACHE) ? true : false;

        // we're gonna use cache;
        $cache = Registry::use('cache');

        // TODO: if cache not exist -> throw Exception
